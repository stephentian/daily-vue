<template lang="html">
  <div>
    ...
    <section class="g-tab">
      <div class="m-tab">
        <a class="jxz tac active" href="javascript:;">进行中</a>
        <router-link class="ywj tac" :to="{ name: 'beted'}">已完结</router-link>
      </div>
    </section>
    <section class="g-betchase-lists" style="overflow:scroll">
      <mt-loadmore :bottom-method="loadBottom" :bottom-all-loaded="allLoaded" :auto-fill="false" :bottomPullText="bottomText" ref="loadmore">
        <router-link tag="aside" class="m-betchase-list" v-for="(order, index) in order_list" :key="index"
         :to="{ path: '/' + order.lottery_alias + 'More', query: { order_id: order.order_id, follow_num: order.follow_num, follow_remain_num: order.follow_remain_num, follow_status: 1 }}">
           <div class="tal">
             <h5 class="gray3">{{ getName(order.lottery_alias) }}</h5>
             <p class="gray9 mt5">{{ getTime(order.order_date) }}</p>
           </div>
          <div class="tar">
            <h5 class="gray3">投注{{ getMoney(order.total_money) }}元</h5>
...
export default {
  ...
  data: () => ({
    chase_order_info: {
      ...
      limit: 10,
      page: 1,
      follow_status: 1 // 0：已停止，1：正常追号，2：已完成
    },
    order_id: 111,
    is_more: false,
