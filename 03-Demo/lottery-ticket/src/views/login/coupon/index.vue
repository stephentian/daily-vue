<template lang="html">
  <div>
    ...
    <section class="g-tab">
      <div class="m-tab">
        <a class="ky tac active" href="javascript:;">可用</a>
        <router-link class="ls tac" :to="{ name: 'couponUsed'}">历史</router-link>
      </div>
    </section>
    <section class="g-usable-coupon">
      <mt-loadmore :bottom-method="loadBottom" :bottom-all-loaded="allLoaded" :auto-fill="false" :bottomPullText="bottomText" ref="loadmore">
        <aside class="m-coupon-new mb15" :class="getCouponState(getTime(coupon.start_date))" v-for="(coupon, index) in coupon_list" :key="index">
          <div>
...
export default {
  ...
  methods: {
    ...
    getCouponState (start_time) {
      let time_state = this.compareDate(start_time)
      // 开始时间大于现在
      if (time_state > 0) {
        return 'unenforced'
      } else {
    ...
    loadBottom () {
      if (this.coupon_count < 10) {
        ...
        this.coupon_use_info.limit += 10
        if (this.coupon_use_info.limit > 40) {
          // 调用加载完成事件
          this.$refs.loadmore.onBottomLoaded()
          this.allLoaded = true
        }
        setTimeout(() => {
          this.$store.dispatch('getCouponList', this.coupon_use_info)
        }, 1000)
      }
    }
  },
