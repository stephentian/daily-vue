<template>
  <div id="bt2Jxk3">
    <section class="g-bet-box">
      <div class="m-bet-box">
        <h5 class="tar">
          <span class="red-circle fl"></span><span class="tal fl h5-txt">选<span class="red">2</span>个不同号码，猜中开奖的任意<span>2</span>位即中<span class="red">8</span>元</span>
        ...
        <div class="balls mt10 mb layoutFlex" :class="eyeShow ? '' : 'mark'">
          <em class="white-rectangle red" v-for="(item, index) in two" :key="index" :class="{selected:item.isSelected}" @click="two_select(item, index)"><span>{{item.num}}</span><mark>{{trendData[index]}}</mark></em>
        </div>
      </div>
    </section>
...
export default {
  ...
  methods: {
    ...
    two_select: function (item) {
      ...
      if (item.isSelected) {
        ...
        for (let i = 0; i < this.two_selected.length; i++) {
          (this.two_selected[i] === item.num) && (this.two_selected.splice(i, 1))
        }
      }
      this.check()
    },
    ...
    submitFail () {
      this.$refs.bTips.bottompopTips('至少选择二个号码')
    },
    submit: function () {
      if (this.isDigClicked) {
        ...
        let beforeDone = () => (new Promise((resolve, reject) => {
          ...
          // jxk3Obj.ante_code = this.two_selected
          ...
          if (this.two_selected.length === 2) {
            ...
            jxk3Obj.lottery_alias = 'JXK3'
            jxk3Obj.lottery_period = this.lottery_period
            jxk3Obj.ante_code = this.Coms(this.two_selected)
            jxk3Obj.count = this.count
            jxk3Obj.single_money = this.money
            if (localStorage.token) {
              store.commit('Jxk3', jxk3Obj)
            ...
            let two_selected_arr = this.bonusCombination(this.two_selected, 2)
            ...
            for (let i = 0; i < two_selected_arr.length; i++) {
              ...
              jxk3Obj.lottery_alias = 'JXK3'
              jxk3Obj.lottery_period = this.lottery_period
              jxk3Obj.ante_code = this.Coms(two_selected_arr[i])
              jxk3Obj.count = 1
