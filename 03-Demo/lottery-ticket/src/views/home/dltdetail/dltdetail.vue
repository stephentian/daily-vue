    ...
    <section class="g-detail-bet">
      <div class="m-detail-bet">
        ...
        <div class="m-detail-txt mt5">
          ...
          <p class="mr10"><span v-if="order_status === 0" @click="payOrder" class="mr10 m-detail-btn tac red">立即支付</span><span @click="deleteOrder" class="mr10 m-detail-btn tac" v-if="(order_status !== 2) && (order_status !== 1) && (order_status !== 3) && (order_status !== 7)">删除订单</span></p>
        </div>
        <div class="m-detail-txt mt5">
          <p class="mr10">
...
<!--     <section class="g-open-status mt15">
      <div class="m-input">
        <label>订单状态</label><code>{{ order_status }}</code>
      </div>
      <div class="m-input">
        <label>预计开奖</label><code>今天 21:15</code>
      </div>
      <div class="m-input">
        <label>预计派奖</label><code>今天 23:10</code>
      </div>
    </section> -->
    <section class="g-detail-selected mt15">
      ...
      <!-- <aside class="m-detail-selected" v-for="(item,index) in order_dlt_com">
        <span class="mr10">{{ play_type }}{{ amount }}倍</span>
        <div class="group">
          <em class="red mr5" v-for="(ball, i) in item.reds_selected">{{ ball }}</em>
          <em class="mr5">#</em>
          <em class="blue mr5" v-for="(ball, i) in item.blues_selected">{{ ball }}</em>
        </div>
      </aside>
      <aside class="m-detail-selected" v-for="(item,index) in order_dlt_dt">
        <span class="mr10">{{ play_type }}{{ amount }}倍</span>
        <div class="group">
          <em class="red mr5" v-for="(ball, i) in item.reds_dan_selected">{{ ball }}</em>
          <em class="mr5">$</em>
          <em class="red mr5" v-for="(ball, i) in item.reds_tuo_selected">{{ ball }}</em>
          <em class="mr5">#</em>
          <em class="blue mr5" v-for="(ball, i) in item.blues_tuo_selected">{{ ball }}</em>
        </div>
      </aside> -->
    </section>
    <section>
    ...
    <h5 class="m-title-h5 tac mt5">
      ...
      <router-link class="blue tdl" to="/dltWay">中奖如何计算？</router-link>
    </h5>
    ...
    <footer class="m-footer box-shadow">
      ...
      <div class="m-bottom-btn" v-else>
        ...
        <router-link tag="button" class="m-bottom-btn-white" to="/dlt">大乐透投注</router-link>
      </div>
    </footer>
...
export default {
  ...
  computed: {
    ...mapState({
      ...
      lottery_period_new: state => state.period.lotteryStopTime.lottery_period,
      bonus_code: state => {
  ...
  methods: {
    ...
    submit () {
      ...
      //       // ante_code: "01,02,03,04,05#06,07",
      //       // ante_code: this.dlt[0].ante_code.join(","),
      //       // play_type: "101",
      //       amount: this.amount,
      ...
      if (this.isDigClicked) {
        ...
        let beforeDone = () => (new Promise((resolve, reject) => {
          ...
          this.tickets.forEach(item => {
            ...
            dltObj.play_type = play_type
            dltObj.ante_info = ante_info
            dltObj.lottery_alias = 'dlt'
            dltObj.lottery_period = this.lottery_period
            dltObj.ante_code = ante_code.split(',')
            if (play_type === '101' || play_type === '102' || play_type === '106') {
              dltObj.single_money = item.total_money / 100
            } else if (play_type === '103' || play_type === '104' || play_type === '107') {
              dltObj.single_money = item.total_money / 150
            }

            if (play_type === '101' || play_type === '102' || play_type === '103' || play_type === '104') {
              let reds_selected = ante_code.substring(0, ante_code.indexOf('#')).split(',')
              let blues_selected = ante_code.substring(ante_code.indexOf('#') + 1).split(',')
              if (reds_selected.length > 5) {
                this.count = (blues_selected.length > 2) ? this.combinations(reds_selected.length, 5) * this.combinations(blues_selected.length, 2) : this.combinations(reds_selected.length, 5)
              } else {
                this.count = (blues_selected.length > 2) ? this.combinations(blues_selected.length, 2) : 1
              }

              dltObj.reds_selected = reds_selected
              dltObj.blues_selected = blues_selected
              dltObj.count = this.count
            } else if (play_type === '106' || play_type === '107') {
              let reds_dan_selected = ante_code.substring(0, ante_code.indexOf('$')).split(',')
              let reds_tuo_selected = ante_code.substring(ante_code.indexOf('$') + 1, ante_code.indexOf('#')).split(',')
              let blues_dan_selected = ante_code.substring(ante_code.indexOf('#') + 1, ante_code.lastIndexOf('$')).split(',')
              let blues_tuo_selected = ante_code.substring(ante_code.lastIndexOf('$') + 1).split(',')
              this.count = (parseInt(reds_dan_selected.length) + parseInt(reds_tuo_selected.length) > 5) ? this.combinations(reds_tuo_selected.length, (5 - parseInt(reds_dan_selected.length))) * blues_tuo_selected.length : blues_tuo_selected.length

              dltObj.reds_dan_selected = reds_dan_selected
              dltObj.reds_tuo_selected = reds_tuo_selected
              dltObj.blues_dan_selected = blues_dan_selected
              dltObj.blues_tuo_selected = blues_tuo_selected
              dltObj.count = this.count
            }

            // console.log("dltObj", JSON.parse(JSON.stringify(dltObj)));
