  ...
  <div id="rx4Dlc">
    <section class="g-bet-box">
      <div class="m-bet-box">
        ...
        <h5 class="tar">
          <span class="red-circle fl"></span><span class="tal fl h5-txt">至少选<span class="red">4</span>个号码，猜对任意<span class="red">4</span>个开奖号码即中<span class="red">78</span>元</span>
        ...
        <div class="balls mt10 mb" :class="eyeShow ? '' : 'mark'">
          <em class="white-ball red" v-for="(item, index) in four" :key="index" :class="{selected:item.isSelected}" @click="four_select(item, index)"><span>{{item.num}}</span><mark>{{trendData[index]}}</mark></em>
        </div>
      </div>
    </section>
...
export default {
  ...
  methods: {
    init: function () {
      ...
      for (let i = 1; i < 12; i++) {
        (i < 10) && (i = '0' + i)
        four.push({num: i, isSelected: false})
      }
      this.$set(this, 'four', four)
      this.four_selected.splice(0, this.four_selected.length)
      this.check()
    ...
    check: function () {
      if (this.four_selected.length >= 4) {
        this.$set(this, 'canSubmit', true)
        if (this.four_selected.length > 4) {
          this.count = this.combinations(this.four_selected.length, 4)
        } else {
          this.count = 1
        }
    ...
    four_select: function (item) {
      item.isSelected = !item.isSelected
      if (item.isSelected) {
        if (this.four_selected.length < 11) {
          this.four_selected.push(item.num)
        } else {
          item.isSelected = !item.isSelected
          this.$refs.bTips.bottompopTips('至多投注11个球')
        }
      } else {
        for (let i = 0; i < this.four_selected.length; i++) {
          (String(this.four_selected[i]) === String(item.num)) && (this.four_selected.splice(i, 1))
        }
      }
      this.check()
    },
    randomBalls: function () {
      this.init()
      let result = this.randomArr(4)
      for (let i in result) {
        let num = parseInt(result[i])
        this.four_select(this.four[num])
      }
      this.check()
    },
    randomArr (sum) {
    ...
    submitFail () {
      this.$refs.bTips.bottompopTips('至少选择4个号码')
    },
    submit: function () {
      if (this.isDigClicked) {
        ...
        let beforeDone = () => (new Promise((resolve, reject) => {
          ...
          dlcObj.ante_code = this.four_selected
