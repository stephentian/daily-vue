  ...
  <div id="rx7Dlc">
    <section class="g-bet-box">
      <div class="m-bet-box">
        ...
        <h5 class="tar">
          <!-- <span class="tal fl h5-txt">至少选7个号码，选号包含5个开奖号即中26元</span> -->
          <span class="red-circle fl"></span><span class="tal fl h5-txt">至少选<span class="red">7</span>个号码，猜对任意<span class="red">5</span>个开奖号码即中<span class="red">26</span>元</span>
        ...
        <div class="balls mt10 mb" :class="eyeShow ? '' : 'mark'">
          <em class="white-ball red" v-for="(item, index) in seven" :key="index" :class="{selected:item.isSelected}" @click="seven_select(item, index)"><span>{{item.num}}</span><mark>{{trendData[index]}}</mark></em>
        </div>
      </div>
    </section>
...
export default {
  ...
  methods: {
    init: function () {
      ...
      for (let i = 1; i < 12; i++) {
        (i < 10) && (i = '0' + i)
        seven.push({num: i, isSelected: false})
      }
      this.$set(this, 'seven', seven)
      this.seven_selected.splice(0, this.seven_selected.length)
      this.check()
    ...
    check: function () {
      if (this.seven_selected.length >= 7) {
        this.$set(this, 'canSubmit', true)
        if (this.seven_selected.length > 7) {
          this.count = this.combinations(this.seven_selected.length, 7)
        } else {
          this.count = 1
        }
    ...
    seven_select: function (item) {
      item.isSelected = !item.isSelected
      if (item.isSelected) {
        if (this.seven_selected.length < 11) {
          this.seven_selected.push(item.num)
        } else {
          item.isSelected = !item.isSelected
          this.$refs.bTips.bottompopTips('至多投注11个球')
        }
      } else {
        for (let i = 0; i < this.seven_selected.length; i++) {
          (String(this.seven_selected[i]) === String(item.num)) && (this.seven_selected.splice(i, 1))
        }
      }
      this.check()
    },
    randomBalls: function () {
      this.init()
      let result = this.randomArr(7)
      for (let i in result) {
        let num = parseInt(result[i])
        this.seven_select(this.seven[num])
      }
      this.check()
    },
    randomArr (sum) {
    ...
    submitFail () {
      this.$refs.bTips.bottompopTips('至少选择7个号码')
    },
    submit: function () {
      if (this.isDigClicked) {
        ...
        let beforeDone = () => (new Promise((resolve, reject) => {
          ...
          dlcObj.ante_code = this.seven_selected
