  ...
  <div id="rx5Dlc">
    <section class="g-bet-box">
      <div class="m-bet-box">
        ...
        <h5 class="tar">
          <!-- <span class="tal fl h5-txt">至少选5个号码，猜对任意5个开奖号码即中540元</span> -->
          <span class="red-circle fl"></span><span class="tal fl h5-txt">至少选<span class="red">5</span>个号码，猜对任意<span class="red">5</span>个开奖号码即中<span class="red">540</span>元</span>
        ...
        <div class="balls mt10 mb" :class="eyeShow ? '' : 'mark'">
          <em class="white-ball red" v-for="(item, index) in five" :key="index" :class="{selected:item.isSelected}" @click="five_select(item, index)"><span>{{item.num}}</span><mark>{{trendData[index]}}</mark></em>
        </div>
      </div>
    </section>
...
export default {
  ...
  methods: {
    init: function () {
      ...
      for (let i = 1; i < 12; i++) {
        (i < 10) && (i = '0' + i)
        five.push({num: i, isSelected: false})
      }
      this.$set(this, 'five', five)
      this.five_selected.splice(0, this.five_selected.length)
      this.check()
    ...
    check: function () {
      if (this.five_selected.length >= 5) {
        this.$set(this, 'canSubmit', true)
        if (this.five_selected.length > 5) {
          this.count = this.combinations(this.five_selected.length, 5)
        } else {
          this.count = 1
        }
    ...
    five_select: function (item) {
      item.isSelected = !item.isSelected
      if (item.isSelected) {
        if (this.five_selected.length < 11) {
          this.five_selected.push(item.num)
        } else {
          item.isSelected = !item.isSelected
          this.$refs.bTips.bottompopTips('至多投注11个球')
        }
      } else {
        for (let i = 0; i < this.five_selected.length; i++) {
          (String(this.five_selected[i]) === String(item.num)) && (this.five_selected.splice(i, 1))
        }
      }
      this.check()
    },
    randomBalls: function () {
      this.init()
      let result = this.randomArr(5)

      for (let i in result) {
        let num = parseInt(result[i])
        this.five_select(this.five[num])
      }
      this.check()
    },
    randomArr (sum) {
    ...
    submitFail () {
      this.$refs.bTips.bottompopTips('至少选择5个号码')
    },
    submit: function () {
      if (this.isDigClicked) {
        ...
        let beforeDone = () => (new Promise((resolve, reject) => {
          ...
          dlcObj.ante_code = this.five_selected
