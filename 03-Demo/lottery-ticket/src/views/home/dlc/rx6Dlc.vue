  ...
  <div id="rx6Dlc">
    <section class="g-bet-box">
      <div class="m-bet-box">
        ...
        <h5 class="tar">
          <!-- <span class="tal fl h5-txt">至少选6个号码，选号包含5个开奖号即中90元</span> -->
          <span class="red-circle fl"></span><span class="tal fl h5-txt">至少选<span class="red">6</span>个号码，猜对任意<span class="red">5</span>个开奖号码即中<span class="red">90</span>元</span>
        ...
        <div class="balls mt10 mb" :class="eyeShow ? '' : 'mark'">
          <em class="white-ball red" v-for="(item, index) in six" :key="index" :class="{selected:item.isSelected}" @click="six_select(item, index)"><span>{{item.num}}</span><mark>{{trendData[index]}}</mark></em>
        </div>
      </div>
    </section>
...
export default {
  ...
  methods: {
    init: function () {
      ...
      for (let i = 1; i < 12; i++) {
        (i < 10) && (i = '0' + i)
        six.push({num: i, isSelected: false})
      }
      this.$set(this, 'six', six)
      this.six_selected.splice(0, this.six_selected.length)
      this.check()
    ...
    check: function () {
      if (this.six_selected.length >= 6) {
        this.$set(this, 'canSubmit', true)
        if (this.six_selected.length > 6) {
          this.count = this.combinations(this.six_selected.length, 6)
        } else {
          this.count = 1
        }
    ...
    six_select: function (item) {
      item.isSelected = !item.isSelected
      if (item.isSelected) {
        if (this.six_selected.length < 11) {
          this.six_selected.push(item.num)
        } else {
          item.isSelected = !item.isSelected
          this.$refs.bTips.bottompopTips('至多投注11个球')
        }
      } else {
        for (let i = 0; i < this.six_selected.length; i++) {
          (String(this.six_selected[i]) === String(item.num)) && (this.six_selected.splice(i, 1))
        }
      }
      this.check()
    },
    randomBalls: function () {
      this.init()
      let result = this.randomArr(6)

      for (let i in result) {
        let num = parseInt(result[i])
        this.six_select(this.six[num])
      }
      this.check()
    },
    randomArr (sum) {
    ...
    submitFail () {
      this.$refs.bTips.bottompopTips('至少选择6个号码')
    },
    submit: function () {
      if (this.isDigClicked) {
        ...
        let beforeDone = () => (new Promise((resolve, reject) => {
          ...
          dlcObj.ante_code = this.six_selected
