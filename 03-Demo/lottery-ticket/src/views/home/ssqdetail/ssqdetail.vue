<template lang="html">
  <div id="ssqdetail">
    ...
    <section class="g-detail-bet">
      <div class="m-detail-bet">
        <div class="m-detail-title">
          <i class="ico-small lottery-ssq"></i>
          <h3 class="ml10">双色球</h3>
          <span class="ml15">第{{ lottery_period }}期</span>
        </div>
    ...
    <h5 class="m-title-h5 tac mt5">
      <router-link class="blue tdl" to="/ssqWay">中奖如何计算？</router-link>
    </h5>
    ...
    <footer class="m-footer box-shadow">
      ...
      <div class="m-bottom-btn" v-else>
        ...
        <router-link tag="button" class="m-bottom-btn-white" to="/ssq">双色球投注</router-link>
      </div>
    </footer>
      ...
      if (this.isDigClicked) {
        ...
        let beforeDone = () => (new Promise((resolve, reject) => {
          ...
          this.tickets.forEach(item => {
            ...
            ssqObj.play_type = play_type
            ssqObj.ante_info = ante_info
            ssqObj.lottery_alias = 'ssq'
            ssqObj.lottery_period = this.lottery_period
            ssqObj.ante_code = ante_code.split(',')
            ssqObj.single_money = item.total_money / 100

            if (play_type === '101' || play_type === '102') {
              ...
              this.count = (reds_selected.length > 6) ? this.combinations(reds_selected.length, 6) * blues_selected.length : blues_selected.length

              ssqObj.reds_selected = reds_selected
              ssqObj.blues_selected = blues_selected
              ssqObj.count = this.count
            } else if (play_type === '103') {
              ...
              let blues_tuo_selected = ante_code.substring(ante_code.indexOf('#') + 1).split(',')
              this.count = (parseInt(reds_dan_selected.length) + parseInt(reds_tuo_selected.length) > 6) ? this.combinations(reds_tuo_selected.length, 6 - parseInt(reds_dan_selected.length)) * blues_tuo_selected.length : blues_tuo_selected.length

              ssqObj.reds_dan_selected = reds_dan_selected
              ssqObj.reds_tuo_selected = reds_tuo_selected
              ssqObj.blues_tuo_selected = blues_tuo_selected
              ssqObj.count = this.count
            }

            if (localStorage.token) {
              store.commit('Ssq', JSON.parse(JSON.stringify(ssqObj)))
            }
          })
          resolve()
        }))
