...
<!--     <section class="g-open-status mt15">
    ...
    <h5 class="m-title-h5 tac mt5">
      <router-link class="blue tdl" to="/fc3dWay">中奖如何计算？</router-link>
    </h5>
    ...
    <footer class="m-footer box-shadow">
      ...
      <div v-else class="m-bottom-btn">
        ...
        <router-link tag="button" class="m-bottom-btn-white" to="/3d">福彩3D投注</router-link>
      </div>
    </footer>
...
export default {
  ...
  methods: {
    ...
    submit () {
      ...
      //       // ante_code: this.d3[0].ante_code.join(","),
      //       // play_type: "201",
      //       amount: this.amount,
      ...
      if (this.isDigClicked) {
        ...
        let beforeDone = () => (new Promise((resolve, reject) => {
          ...
          this.tickets.forEach(item => {
            ...
            d3Obj.play_type = play_type
            d3Obj.ante_info = ante_info
            d3Obj.lottery_alias = '3d'
            d3Obj.lottery_period = this.lottery_period
            d3Obj.ante_code = ante_code.split(',')
            d3Obj.single_money = item.total_money / 100
            d3Obj.count = 1
            // console.log("d3Obj", d3Obj);
